import com.bridgecanada.utils.HexConversions._

val cipherText =
      List( "",
            "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f",
            "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb",
            "",
            "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070",
            "",
            "",
            "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3",
            "",
            "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83")

val targetCypertext = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"




//val xorFreqTable = xors.flatMap(x => x.fromHexStringToInts.zipWithIndex)
//  .groupBy(identity)
//  .mapValues(_.size)
//  .toSeq
//  .sortWith(_._2 > _._2)
//  .filter(_._2 > 10)
//
//val highFreqXors = xorFreqTable.map(x => x._1._2)



//val origMsg = cipherText.map((x: String) => (testKey.asAsciiHexString ^ x).readable)

val spaceMessage = new StringBuilder(" " * 100).toString.fromAsciiToHexString
var testMessageXor = spaceMessage ^ spaceMessage
var testCipher1 = 3
var testCipher2 = 4

var testCipherXor = cipherText(testCipher1) ^ cipherText(testCipher2)

//val xors: List[String] = for {
//      x <- cipherText
//      y <- cipherText if x != y
//} yield x ^ y

println("          1         2         3         4         5         6         7         8         9         \r\n"
  +"0123456789" * 10)

// take the XOR of all-space messages and compare against enc1 xor enc2
// to find where the likely commonality of space-chars is in the two unknown messages

println(testMessageXor.findMatchesWith(testCipherXor))

var charAt = 13
//println((cipherText(1) ^ spaceMessage)(charAt))
var comp1 = (cipherText(testCipher1) ^ spaceMessage).fromHexStringToInts.toArray
println(comp1(charAt))
var comp2 = (cipherText(testCipher2) ^ spaceMessage).fromHexStringToInts.toArray
println(comp2(charAt))
//((cipherText(1) ^ testKey).fromHexStringToInts.toArray) (30)
//"WEF"
//println((cipherText(1) ^ testKey)(30))
//var result = xors.map(xor => xor.compareAgainst(testXor))
//cipherText.map((x: String) => (testKey ^ x).readable).foreach(x => println(x))
//xors.map((x: String) => (testKey ^ x).readable).foreach(x => println(x))
//var mutTestKey = new StringBuilder("the" * 100)


var mutTestKey: Array[Int] = Array.fill(256){0}
//List(0,9,32,47).foreach(mutTestKey.setCharAt(_,'e'))
//List(1,10,15,23,31,36,40,52,58,64,74).foreach(mutTestKey.setCharAt(_,' '))
//List(3).foreach(mutTestKey.setCharAt(_,' '))

mutTestKey(70) = 156   // 0 + 1 // GOOD
mutTestKey(20) = 170   // 1 + 2 // GOOD
mutTestKey(57) = 232   // 1 + 2 // GOOD
mutTestKey(3) = 137   // 2 + 3 // GOOD

mutTestKey(30) = 145   // 0 + 1
mutTestKey(53) = 116   // 0 + 1
mutTestKey(0) = 14   // 2 + 3
mutTestKey(1) = 113   // 2 + 3
mutTestKey(2) = 43   // 2 + 3
mutTestKey(14) = 149   // 3 + 4
//mutTestKey(1) = 14   // 3 + 4

//val testKey = mutTestKey.toString.asAsciiHexString
//String.format("%02X", value);
//val testKey = mutTestKey.map(Integer.toHexString).mkString
val testKey = mutTestKey.map("%02x".format(_)).mkString
//1.map(Integer.toHexString).mkString
//val paddedHex = String.format("%02X", 1)


println("          1         2         3         4         5         6         7         8         9         \r\n"
  +"0123456789" * 10)
//testKey ^ cipherText(2)
//println((cipherText(1) ^ testKey).readable)
//println("OK")
cipherText.map((x: String) => (testKey ^ x).readable).foreach(x => println(x))
